-- VHDL Entity MIPS.MIPS_tb.symbol
--
-- Created:
--          by - kolaman.UNKNOWN (KOLAMAN-PC)
--          at - 09:22:45 17/02/2013
--
-- Generated by Mentor Graphics' HDL Designer(TM) 2011.1 (Build 18)
--


ENTITY MIPS_tb IS
-- Declarations

END MIPS_tb ;

--
-- VHDL Architecture MIPS.MIPS_tb.struct
--
-- Created:
--          by - kolaman.UNKNOWN (KOLAMAN-PC)
--          at - 09:22:45 17/02/2013
--
-- Generated by Mentor Graphics' HDL Designer(TM) 2011.1 (Build 18)
--
LIBRARY IEEE;
USE IEEE.STD_LOGIC_1164.ALL;
USE IEEE.STD_LOGIC_ARITH.ALL;

LIBRARY work;

ARCHITECTURE struct OF MIPS_tb IS

   -- Architecture declarations

   -- Internal signal declarations
   SIGNAL ALU_result_out  : STD_LOGIC_VECTOR( 31 DOWNTO 0 );
   SIGNAL Branch_out      : STD_LOGIC;
   SIGNAL Instruction_out : STD_LOGIC_VECTOR( 31 DOWNTO 0 );
   SIGNAL Memwrite_out    : STD_LOGIC;
   SIGNAL PC              : STD_LOGIC_VECTOR( 10 DOWNTO 0 );	-- 11 BITS
   SIGNAL Regwrite_out    : STD_LOGIC;
   SIGNAL Zero_out        : STD_LOGIC;
   SIGNAL clock           : STD_LOGIC;
   SIGNAL read_data_1_out : STD_LOGIC_VECTOR( 31 DOWNTO 0 );
   SIGNAL read_data_2_out : STD_LOGIC_VECTOR( 31 DOWNTO 0 );
   SIGNAL reset           : STD_LOGIC;
   SIGNAL write_data_out  : STD_LOGIC_VECTOR( 31 DOWNTO 0 );

	SIGNAL KEY1				: STD_LOGIC;
	SIGNAL KEY2				: STD_LOGIC;
	SIGNAL KEY3				: STD_LOGIC;
	SIGNAL Switches	   		: STD_LOGIC_VECTOR (7 DOWNTO 0);
	SIGNAL LEDG_out	   		: STD_LOGIC_VECTOR (7 DOWNTO 0);
	SIGNAL LEDR_out	   		: STD_LOGIC_VECTOR (7 DOWNTO 0);
	SIGNAL HEX0_out	   		: STD_LOGIC_VECTOR (6 DOWNTO 0);		-- converting to 7-seg
	SIGNAL HEX1_out	  		: STD_LOGIC_VECTOR (6 DOWNTO 0);		-- converting to 7-seg
	SIGNAL HEX2_out	  		: STD_LOGIC_VECTOR (6 DOWNTO 0);		-- converting to 7-seg
	SIGNAL HEX3_out	  		: STD_LOGIC_VECTOR (6 DOWNTO 0);			-- converting to 7-seg				
	SIGNAL Zero4LEDR		: STD_LOGIC_VECTOR (1 DOWNTO 0);			

   -- Component Declarations
   COMPONENT MIPS
   GENERIC (n : INTEGER := 1);
   PORT (
      clk_24Mhz       : IN     STD_LOGIC;
      reset           : IN     STD_LOGIC;
      ALU_result_out  : OUT    STD_LOGIC_VECTOR ( 31 DOWNTO 0 );
      Branch_out      : OUT    STD_LOGIC;
      Instruction_out : OUT    STD_LOGIC_VECTOR ( 31 DOWNTO 0 );
      Memwrite_out    : OUT    STD_LOGIC;
      PC              : OUT    STD_LOGIC_VECTOR ( 10 DOWNTO 0 );	-- 11 BITS
      Regwrite_out    : OUT    STD_LOGIC;
      Zero_out        : OUT    STD_LOGIC;
      read_data_1_out : OUT    STD_LOGIC_VECTOR ( 31 DOWNTO 0 );
      read_data_2_out : OUT    STD_LOGIC_VECTOR ( 31 DOWNTO 0 );
      write_data_out  : OUT    STD_LOGIC_VECTOR ( 31 DOWNTO 0 );
	  ENABLE			: IN STD_LOGIC;
	  KEY1				: IN STD_LOGIC;			
	  KEY2				: IN STD_LOGIC;	
	  KEY3				: IN STD_LOGIC;	
	  Switches	   		: IN STD_LOGIC_VECTOR (7 DOWNTO 0);
	  LEDG_out	   		: OUT STD_LOGIC_VECTOR (7 DOWNTO 0);
	  LEDR_out	   		: OUT STD_LOGIC_VECTOR (7 DOWNTO 0);
	  HEX0_out	   		: OUT STD_LOGIC_VECTOR (6 DOWNTO 0);		-- converting to 7-seg
	  HEX1_out	  		: OUT STD_LOGIC_VECTOR (6 DOWNTO 0);		-- converting to 7-seg
	  HEX2_out	  		: OUT STD_LOGIC_VECTOR (6 DOWNTO 0);		-- converting to 7-seg
	  HEX3_out	  		: OUT STD_LOGIC_VECTOR (6 DOWNTO 0);		-- converting to 7-seg	
	  Zero4LEDR			: OUT STD_LOGIC_VECTOR (1 DOWNTO 0)
   );
   END COMPONENT;
   
   COMPONENT MIPS_tester
   PORT (
      ALU_result_out  : IN     STD_LOGIC_VECTOR ( 31 DOWNTO 0 );
      Branch_out      : IN     STD_LOGIC ;
      Instruction_out : IN     STD_LOGIC_VECTOR ( 31 DOWNTO 0 );
      Memwrite_out    : IN     STD_LOGIC ;
      PC              : IN     STD_LOGIC_VECTOR ( 10 DOWNTO 0 );	-- 11 BITS
      Regwrite_out    : IN     STD_LOGIC ;
      Zero_out        : IN     STD_LOGIC ;
      read_data_1_out : IN     STD_LOGIC_VECTOR ( 31 DOWNTO 0 );
      read_data_2_out : IN     STD_LOGIC_VECTOR ( 31 DOWNTO 0 );
      write_data_out  : IN     STD_LOGIC_VECTOR ( 31 DOWNTO 0 );
      clock           : OUT    STD_LOGIC ;
      reset           : OUT    STD_LOGIC 
   );
   END COMPONENT;

   -- Optional embedded configurations
   -- pragma synthesis_off
   FOR ALL : MIPS USE ENTITY work.mips;
   FOR ALL : MIPS_tester USE ENTITY work.mips_tester;
   -- pragma synthesis_on


BEGIN

   -- Instance port mappings.
   U_0 : MIPS
	GENERIC MAP (0)			-- Make the MIPS right to ModelSim
      PORT MAP (
         reset           => reset,
         clk_24Mhz       => clock,
         PC              => PC,
         ALU_result_out  => ALU_result_out,
         read_data_1_out => read_data_1_out,
         read_data_2_out => read_data_2_out,
         write_data_out  => write_data_out,
         Instruction_out => Instruction_out,
         Branch_out      => Branch_out,
         Zero_out        => Zero_out,
         Memwrite_out    => Memwrite_out,
         Regwrite_out    => Regwrite_out,
		 ENABLE			 => '0',
		 KEY1			 => '1',
		 KEY2			 => '1',
		 KEY3			 => '1',
		 Switches		 => X"06",		-- TODO: should be switches
		 LEDG_out		 => LEDG_out,
		 LEDR_out		 => LEDR_out,
		 HEX0_out		 => HEX0_out,
		 HEX1_out		 => HEX1_out,
		 HEX2_out		 => HEX2_out,
		 HEX3_out		 => HEX3_out
      );
	
	  
   U_1 : MIPS_tester
      PORT MAP (
         ALU_result_out  => ALU_result_out,
         Branch_out      => Branch_out,
         Instruction_out => Instruction_out,
         Memwrite_out    => Memwrite_out,
         PC              => PC,
         Regwrite_out    => Regwrite_out,
         Zero_out        => Zero_out,
         read_data_1_out => read_data_1_out,
         read_data_2_out => read_data_2_out,
         write_data_out  => write_data_out,
         clock           => clock,
         reset           => reset
      );

END struct;
